...
bool vis[MAXV];
pair<long long, int> cost_a2u[MAXV]; //developed over dijkstra
pair<long long, int> cost[MAXV][MAXV]; //precomputed {cost, flights}
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = 0; j < route_len; j++)
        {
            ...
            for (int k = 0; k < j; k++)
            {
                cost[route[k]][route[j]] = min(cost[route[k]][route[j]], make_pair(route_cost, j - k));
            }
        }
    }
